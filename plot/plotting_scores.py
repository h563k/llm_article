...
def plot_scores(score_df, section, model_list):
    """
    ...
    for group_idx, model_group in enumerate(model_groups):
        ...
        # 设置标题和标签
        group_suffix = f" ({get_label('组')} {group_idx+1})" if len(
            model_groups) > 1 else ""
        axes[0].set_title(
        ...
        if len(model_groups) > 1:
            output_filename = get_output_path(
                f"{section}_scores_{group_idx+1}.jpg")
        else:
