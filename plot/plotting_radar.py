...
def plot_radar(radar_df, error_types, section, exclude_models=None):
    log_base = 10  # 对数基数
    """
    绘制错误类型雷达图 (改进版)
    ...
    angles = np.linspace(0, 2 * np.pi, num_error_types, endpoint=False).tolist()
    ...
    for i, model in enumerate(models):
        ...
        values = [min_value if v == 0 else np.log(v)/np.log(log_base) for v in values]
    ...
    # 设置标题
    plt.title(f"{get_label(section)} - {get_label('模型错误类型雷达图')} (log10尺度)", size=15, y=1.08)
