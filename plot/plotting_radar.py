...
def plot_radar(radar_df, error_types, section,  model_list, exclude_models=None,):
    ...
    for group_num, model_group in enumerate(groups, 1):
        ...
        # 动态确定最大值
        print('max_value')
        print(np.log(group_df[model_group].max().max())/np.log(log_base))
        max_value = np.ceil(np.log(group_df[model_group].max().max())/np.log(log_base))
        ...
        # 调整布局并保存结果
        plt.tight_layout()
        output_filename = get_output_path(f"{section}_radar{group_num}.jpg")
